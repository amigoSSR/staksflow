...
async function generateWeeklyReports(referenceDate = new Date()) {
  ...
  // ── Fetch all project diaries in the period ────────────────────────────────
  ...
  if (!projectDiaries.length && !generalDiaries.length && !completedProjects.length) {
    return { period: periodLabel, generated_at: new Date(), created_count: 0, reports: [] };
  }

  // ── Group by project → leader → mentees ───────────────────────────────────
  // Map: leaderId → { leaderName, projects: Map<projectId, { projectName, mentees[], diaries[], activities[] }> }
  ...
  projectDiaries.forEach(diary => {
    if (diary.project_id && diary.project) {
      ...
      leaderEntry.projects.get(pid).diaries.push(diary);
    } else {
  ...
  const diaryCount  = projectDiaries.length + generalDiaries.length;
